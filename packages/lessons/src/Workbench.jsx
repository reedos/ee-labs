  ...
  return <div className="lesson-workbench">
    ...
    <main className="lesson-main"><header className="lesson-heading"><span>{lesson.id.toUpperCase()}</span><h2>{lesson.name}</h2></header>
      ...
      <div className="lesson-body">{outcome.error?<section role="alert"><h3>Model boundary</h3><p>{outcome.error}</p><button onClick={reset}>Reset settings</button></section>:<>
        ...
        {view==='Explore'&&<><p>{r.plotNote}</p>{r.plots?.map((plot,i)=>plot.kind==='smith'?<SmithCanvas key={i} {...plot}/>:<Plot key={i} data={plot}/>)}{r.table&&<><p className="lesson-table-hint">On narrow screens, scroll the table sideways to see all columns.</p><div className="lesson-table-wrap" tabIndex={0} role="region" aria-label="Scrollable results table"><table><thead><tr>{r.table.headers.map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{r.table.rows.map((row,i)=><tr key={i}>{row.map((v,j)=><td key={j}>{typeof v==='number'?fmt(v):v}</td>)}</tr>)}</tbody></table></div></>}<h3>What changed?</h3><p>{r.conclusion}</p></>}
        {view==='Practice'&&<Practice key={JSON.stringify(p)} problem={r.practice}/>}
        {view==='Explore'&&r.related&&<section className="lesson-handover"><h3>Related lesson</h3><p>{r.related.note}</p>{siblingUrl(r.related.app,r.related.id)?<a href={siblingUrl(r.related.app,r.related.id)}>{r.related.label}</a>:<p>Open the assembled labs to visit {r.related.label}.</p>}</section>}{view==='Explore'&&(r.handovers??(r.handover?[r.handover]:[])).map((handover,i)=><section className="lesson-handover" key={i}><h3>Continue exploring</h3><p>{handover.note}</p>{siblingUrl(handover.app,buildLink(handover.patch))?<a href={siblingUrl(handover.app,buildLink(handover.patch))}>{handover.label}</a>:<p>Open the labs through the assembled site to use this handover.</p>}</section>)}{view!=='Practice'&&<div className="lesson-results" aria-label="Calculated results">{r.readings.map((v,i)=><div key={i}><span>{v.label}</span><strong>{fmt(v.value)} {v.unit}</strong></div>)}</div>}
      </>}</div>
    </main>
  </div>
}
