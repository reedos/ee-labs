...
// too (`PROGRAM.md` §4). `outputs` draws a Moore output inside the circle,
// which is what an instruction decoder needs, and `encoding` prints each
// state's bits beside its name so that the picture and the equations name the
// same thing. `taken` lights the arc last followed, which the Computer Lab's
// control unit uses to show the step it is in.
//
// The two apps' copies agreed prop for prop, so this file moved unchanged.
// The Logic Lab's own state machines are Group F, which is specified and not
// built (LOGIC_LAB_PLAN.md §9). The Computer Lab draws its multicycle control
// unit on it (COMPUTER_LAB_PLAN.md D2), five states with every class of
// instruction walking the ones it needs.
