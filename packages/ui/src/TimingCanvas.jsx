...
//
// Built by the Logic Lab and promoted here once the Computer Lab claimed it
// too (`PROGRAM.md` §4). It moved from `apps/logic-lab/src/components` as this
// file, unchanged in its drawing, because both apps' copies agreed prop for
// prop.
//
// A waveform from a discrete-event engine is a step function, so it is drawn
// as one. The edges are vertical and nothing is interpolated between two
// transitions, because there is nothing between them (REVIEW_PLAYBOOK §6,
// data against interpolation).
//
// `busses` draws a set of rows as one numeric row and `spans` measures an
// interval, which the Interfaces Lab needs for a protocol. `analog` draws one
// real-valued row against two threshold levels, which is that lab's pin.
// `cursors` is its read pair. `causes` joins each event to the event that
// caused it, which is the VLSI Lab's. `signals`, `busses`, `analog`, `marks`,
// `spans`, `cursors` and `causes` are the Logic Lab's original props, kept
// exactly. The Computer Lab's copy added nothing to the signature: it draws
// its carry walk and its control unit through the same props.
//
// One difference between the two copies, resolved here. Each app quoted its
// axis and label text through its own `ps`-style time formatter, because the
// Computer Lab's engine ticks in a 10 fs grid and converts to picoseconds
// before printing, where the Logic Lab's engine already counts picoseconds.
// That formatter cannot move here as an import, since this package does not
// depend on either lab's engine. It is a prop instead, `fmtTime`, defaulting
// to the Logic Lab's own picosecond formatting so that lab's call sites need
// no change, with the Computer Lab passing its own.
  ...
  const ref = useCanvas(
    (ctx, w) => {
      ...
      for (const sp of geo.spans) {
        ...
        ctx.font = '11px ui-monospace, monospace'
        ctx.fillText(`${sp.label ? `${sp.label}, ` : ''}${fmtTime(sp.to - sp.from)}`, (sx(sp.from) + sx(sp.to)) / 2, y - 8)
      }
      ...
      if (geo.cursors.length === 2) {
        ...
        ctx.fillText(fmtTime(geo.interval), (geo.cursors[0].x + geo.cursors[1].x) / 2, h - BOTTOM + 20)
      }
    ...
    [res, JSON.stringify(signals), JSON.stringify(busses), JSON.stringify(marks), JSON.stringify(spans), JSON.stringify(cursors), causes, t0, t1, cursor, h, fmtTime],
  )
