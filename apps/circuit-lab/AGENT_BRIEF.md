...
- **One lane per agent, one worktree per agent.** `PROGRAM.md` §2: the branch is
  ...
  Never work in `~/projects/ee-labs` itself. Three agents in one tree have
  already crossed streams once: a `git add -A` from one swept another's
  half-finished files into an unrelated commit, and `npm run build` clobbers
  another agent's preview mid-harness.
- **Stage by path, never `git add -A` and never `commit -a`**, even in your
  own worktree, the habit is the protection: `git add apps/circuit-lab`.
