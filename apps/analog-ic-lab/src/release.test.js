...
// While it says `dark`, the lab is built and deployed at /analog-ic-lab/ but
...
const status = read('apps/analog-ic-lab/RELEASE_STATUS').trim()
...
describe(`release status "${status}"`, () => {
  ...
  if (status === 'dark') {
    for (const p of PUBLIC) {
      it(`${p} does not mention the lab`, () => {
        expect(read(p)).not.toMatch(/analog-ic-lab|Analog IC Lab/)
      })
    }
  } else {
    for (const p of PUBLIC) {
      it(`${p} links the lab`, () => {
        expect(read(p)).toMatch(/analog-ic-lab/)
      })
    }
  }
  ...
  it('records the deploy line the director adds, in NEEDS.md', () => {
    expect(read('apps/analog-ic-lab/NEEDS.md')).toMatch(
      /cp -r apps\/analog-ic-lab\/dist _site\/analog-ic-lab/,
    )
  })
  ...
  it('the deploy workflow ships the build either way, so the dark URL exists to review', () => {
    ...
    expect(read('.github/workflows/deploy.yml')).toMatch(/apps\/analog-ic-lab\/dist\s+_site\/analog-ic-lab/)
  })
})
