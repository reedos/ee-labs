...
// While it says `dark`, the lab is built and deployed at /comms-lab/ but
...
const status = read('apps/comms-lab/RELEASE_STATUS').trim()
...
describe(`release status "${status}"`, () => {
  ...
  if (status === 'dark') {
    for (const p of PUBLIC) {
      it(`${p} does not mention the lab`, () => {
        expect(read(p)).not.toMatch(/comms-lab|Communications Lab/)
      })
    }
  } else {
    for (const p of PUBLIC) {
      it(`${p} links the lab`, () => {
        expect(read(p)).toMatch(/comms-lab/)
      })
    }
  }
  ...
  it('records the deploy line the director adds, in NEEDS.md', () => {
    expect(read('apps/comms-lab/NEEDS.md')).toMatch(
