...
// uses the word. No experiment before that one uses it at all, so a reader never
// meets a word the sidebar has not yet been willing to explain.
//
// `MATCH` is how a word is recognised in prose. It is a pattern per term rather
// than the term's own name because the prose says "dielectric" where the term is
// "permittivity", and a rule that only caught the headword would pass a lesson
// that never says it.
 ...
 * ahead may say the word it is pointing at. "As it does for the coaxial cable
 * in B2" is a signpost, not an undefined term: the reader is told where the
 * word will be explained. A use with no such signpost is not excused.
 */
...
/** The terms whose introducer shows the thing without naming it. See the test below. */
const CHIPS = [
  'permittivity@a1',
  'dipole@a2',
  'boundarycondition@c1',
  'conductivity@d1',
  'resistivity@d1',
  'fluxdensity@e1',
  'permeability@e1',
...
describe('terms where they first do work', () => {
  ...
  it('each term is spelled out where it is introduced, or rides as a named chip', () => {
    // A term whose own introducer never says the word is a chip under the note
    // rather than a marked word in it. That is allowed where the SYMBOL or the
    // construction is on screen while the name is not: A1 shows ε₀ without
    // saying permittivity, A2 draws two opposite charges without calling the
    // pair a dipole, E1 reads a field in tesla without naming flux density.
    // The list is written out so that a tenth such term fails this test and has
  ...
  it('the sidebar offers an experiment its own terms, in the order it lists them', () => {
    for (const e of EXPERIMENTS) {
      ...
      expect(offered.map((t) => t.id), `${e.id} terms`).toEqual(e.terms || [])
