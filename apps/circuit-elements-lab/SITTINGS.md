...
The status line above is computed by `src/sittings.js statusLine()` from
`sittings.json`, and `src/sittings.test.js` fails when this file says anything
else. The document cannot get ahead of the record.
...
Start the clock when the experiment is on screen. Stop it at their first act:
a knob moved, the cursor dragged (F3's first step), or the meters switched.
Write their sentence down word for word before deciding whether it matches.

## The seats

Each person sits three experiments: a first look, a method, and a dynamic.
That is nine sittings in all, at least three of them on the phone.
...
- **Recall matches `see`** for at least 8 of the 9. Their one sentence matches
  when it states the claim the experiment's `see` sentence makes, in their
  words rather than the lab's. "The source stays at 12 no matter what the
  resistor does" matches A1. "You turn the knob and the number changes" does
  not. One miss is allowed, and a second blocks the group it happened in.
- **Clarity ≥ 4.5** as the mean of the 1–5 ratings, per experiment. Under it
  blocks that experiment's group.

Below target anywhere, fix what they stumbled on and add the test that would
have caught it. Then repeat that one sitting with a new person.
...
`who` is a label, never a name. `device` is `phone` or `laptop`. `stumbled` is
optional and is the most useful field, because it is the list of the next
fixes. Then run the tests. They check every field, that the experiment is in
the course, and that the status line above matches the record. Update the
