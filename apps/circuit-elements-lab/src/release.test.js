...
// built and deployed at /circuit-elements-lab/ but nothing a visitor sees may
...
const status = read('apps/circuit-elements-lab/RELEASE_STATUS').trim()
  ...
  if (status === 'dark') {
    for (const p of PUBLIC) {
      it(`${p} does not mention the lab`, () => {
        expect(read(p)).not.toMatch(/circuit-elements-lab|Circuit Elements Lab/)
      })
    }
  } else {
    for (const p of PUBLIC) {
      it(`${p} links the lab`, () => {
        expect(read(p)).toMatch(/circuit-elements-lab/)
      })
    }
  }

  it('the deploy workflow ships the build either way, so the dark URL exists to review', () => {
    expect(read('.github/workflows/deploy.yml')).toMatch(/apps\/circuit-elements-lab\/dist\s+_site\/circuit-elements-lab/)
  })
})
