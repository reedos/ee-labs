...
function emiEntry(exp, x) {
  ...
  const harmonics = f.pulse.slice(0, 5).map((h) => ({
    ...
    value: h.peak,
    unit: 'A',
    // A harmonic the closed form puts at exactly zero comes back as a residue,
    // so each figure is read against its own fundamental.
    note: `closed form ${fmtz(h.ideal, 'A', 3, f.pulse[0].ideal)}, at the line ${fmtz(h.line, 'A', 3, f.pulse[0].peak)}`,
  }))
  return {
    blocks: [
      T(
