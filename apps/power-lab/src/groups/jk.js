...
/**
 * A sweep key's exact value at the knob, read off the analysed state the top
 * bar and the note already use, so the marker lands on the setting rather
 * than on the nearest sampled point. The hard-switched reference is solved
 * at the knob the way the sweep solves it at each of its points.
 */
export function exactJkY(key, x, params) {
  const m = x.m
  switch (key) {
    case 'Mn':
      return m.M
    ...
    case 'eta':
      return m.eta
    case 'etaHard':
      return hardReference(resonantParams(params), m).eta
    default:
      return undefined
  }
}
