...
/**
 * The line-to-line fundamental's peak at these knobs, plain and with the
 * third-harmonic offset, each from its own solved steady state. The sweep
 * above samples the same two numbers along the knob's range; the marker on
 * the sweep reads these, so it lands on the setting and not on a neighbour.
 */
export function lineFundamentalPeaks(params) {
  const base = threePhaseParams(params)
  const one = (inject) => {
    const conv = threePhase('spwm3', { ...base, inject })
    return threePhaseMeasures(threePhaseSteadyState(conv), { harmonics: 1, dense: 8 }).Vll1 * Math.SQRT2
  }
  return { plain: one(0), offset: one(INJECTION) }
}
