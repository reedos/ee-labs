...
import { axisFmt, fmtz, niceBounds } from '../format.js'
        ...
        <tbody>
          {f.pulse.slice(0, 7).map((q) => (
            <tr key={q.k}>
              ...
              <td className="num">{fmtz(q.peak, 'A', 4, f.pulse[0].peak)}</td>
              <td className="num">{fmtz(q.ideal, 'A', 4, f.pulse[0].ideal)}</td>
              <td className="num">{fmtz(q.line, 'A', 4, f.pulse[0].peak)}</td>
