...
import { time as ps } from '../format.js'

// The timing diagram: signals down the left, time across, one row per signal
// drawn as the instants at which it changed.
//
// COPIED from apps/logic-lab/src/components/TimingCanvas.jsx, prop for prop.
// That lab built it, `PROGRAM.md` §4 names this lab as its second claimant, and
// neither has been promoted into `packages/ui` yet. `NEEDS.md` §3 asks the
// director for the promotion, and this copy is the debt until that lands. Do
// not edit it here: a change belongs in the promoted component.
//
...
// interpolation).
//
// The props past `signals` were written for labs that did not exist yet
// (LOGIC_LAB_PLAN.md Decision 5, that lab's AGENT_BRIEF.md §3.7). `busses` draws a set of
