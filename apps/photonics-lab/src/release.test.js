...
// The dark launch, enforced — the file every lab in the suite carries, with the
// slug changed. RELEASE_STATUS is one word, `dark` or `released`, and Reed alone
// changes it. While it says `dark`, the lab is built and deployed at
// /photonics-lab/ but nothing a visitor sees may point at it: not the splash
...
const status = read('apps/photonics-lab/RELEASE_STATUS').trim()
...
describe(`release status "${status}"`, () => {
  ...
  if (status === 'dark') {
    for (const p of PUBLIC) {
      it(`${p} does not mention the lab`, () => {
        expect(read(p)).not.toMatch(/photonics-lab|Photonics Lab/)
      })
    }
  } else {
    for (const p of PUBLIC) {
      it(`${p} links the lab`, () => {
        expect(read(p)).toMatch(/photonics-lab/)
      })
    }
  }
  ...
  // exact text the workflow takes. Nothing here reads the workflow, because a
  // lab that has not been integrated yet would be asserting about a file the
  // director has not written its line into.
  const DEPLOY = 'cp -r apps/photonics-lab/dist _site/photonics-lab'

  it('NEEDS.md carries the deploy line, in the text the workflow takes', () => {
    expect(read('apps/photonics-lab/NEEDS.md')).toContain(DEPLOY)
  })
})
