...
// While it says `dark`, the lab is built and deployed at /mixed-signal-lab/ but
...
const status = read('apps/mixed-signal-lab/RELEASE_STATUS').trim()
...
describe(`release status "${status}"`, () => {
  ...
  if (status === 'dark') {
    for (const p of PUBLIC) {
      it(`${p} does not mention the lab`, () => {
        expect(read(p)).not.toMatch(/mixed-signal-lab|Mixed-Signal Lab/)
      })
    }
  } else {
    for (const p of PUBLIC) {
      it(`${p} links the lab`, () => {
        expect(read(p)).toMatch(/mixed-signal-lab/)
      })
    }
  }
  ...
  it('records the deploy line the director adds, in NEEDS.md', () => {
    expect(read('apps/mixed-signal-lab/NEEDS.md')).toMatch(
      /cp -r apps\/mixed-signal-lab\/dist _site\/mixed-signal-lab/,
    )
  })
  ...
  it('the deploy workflow ships the build either way, so the dark URL exists to review', () => {
    ...
    expect(read('.github/workflows/deploy.yml')).toMatch(/apps\/mixed-signal-lab\/dist\s+_site\/mixed-signal-lab/)
  })
})
