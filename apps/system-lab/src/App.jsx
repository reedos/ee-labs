...
import { FlowStrip, LevelsPane, NumbersPane, TablePane } from './components/panes.jsx'
      ...
      {k.kind === 'choice' ? (
        <div className="toggle-knob">
          ...
          <div className="segmented" role="group" aria-label={k.label}>
            {k.options.map((o) => (
              <button key={String(o.value)} type="button" aria-pressed={params[k.key] === o.value} className={params[k.key] === o.value ? 'on' : ''} onClick={() => setParam(k.key, o.value)}>
                {o.label}
              </button>
            ))}
          </div>
      ...
      <main className="views is-single">
        <section className="view is-primary">
          <div className="view-head">
            {x.declined ? null : <FlowStrip x={x} />}
            <ViewSwitch value={currentView} onChange={setView} views={exp.views} />
          </div>
          <div className="view-body">
            {x.declined ? (
              <p className="hint sys-declined" data-role="declined">
                {x.declined.says}
              </p>
            ) : (
