...
export const TERMS_J = {
  ...
  mirror: {
    ...
    def:
      'Two matched transistors sharing a base connection, one of them wired so that its own collector feeds ' +
      'its base. The wired one sets a voltage across both junctions, and the other carries the same current ' +
      'into whatever it is connected to. It is a current source whose value is copied from a reference.',
  },
  activeload: {
    name: 'The active load',
    def:
      'A transistor used in place of a collector resistor. Its small-signal resistance r_o is a hundred times ' +
      'what a resistor of the same DC drop would be, so the gain rises by that factor. The price is a bias ' +
      'point set by two currents matching rather than by a resistor.',
  },
}
...
export const MATCH_J = {
  ...
  mirror: /\bcurrent mirror\b|\bmirror\b/i,
