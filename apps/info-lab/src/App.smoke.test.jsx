...
describe('the shell', () => {
  ...
  it('lists all six groups in the sidebar, including the one that is not built', () => {
    const h = html()
    for (const g of new Set(EXPERIMENTS.map((e) => e.group))) expect(h, g).toContain(g.split(' · ')[1])
    expect(h).toContain('Coding gain measured')
    expect(h).toContain('This group waits on the Communications Lab.')
  })
})
...
const paneOf = (view, x) => {
  ...
  if (view === 'weights')
    return x.block ? (
      <WeightCanvas weights={x.block.weights || []} d={x.block.d} t={x.block.t} detect={x.block.detect} />
    ) : (
      <WeightCanvas weights={(x.conv.spectrum || { a: [] }).a} d={x.conv.dfree} />
    )
