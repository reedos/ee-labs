...
// reveal, no personified decoder — so twenty-one experiments stay in one voice.
...
describe('every group heading names its content', () => {
  // A heading is a title rather than a button label, so it carries the title
  // budget (STYLE.md S9) and not the four-word cap of a control.
  for (const g of new Set(EXPERIMENTS.map((e) => e.group))) {
    it(g, () => expectPlain(g.replace(/^[A-F] · /, ''), 'title', g))
  }
})
