...
// than the term's own name because the prose says "standing-wave ratio" where
// the term is `vswr`, and a rule that only caught the headword would pass a
// lesson that never says it.
 ...
 * ahead may say the word it is pointing at. A use with no such signpost is not
 * excused.
 */
...
describe('terms where they first do work', () => {
  ...
  it('every term some experiment lists is defined, and every definition is listed once', () => {
    ...
    expect(new Set(listed).size, 'a term is listed by two experiments').toBe(listed.length)
  })
  ...
  it('each term is spelled out where it is introduced, or rides as a named chip', () => {
    ...
    // rather than a marked word in it. Nothing in these two groups is one, and
    // the empty list is written out so that the first such term has to be
