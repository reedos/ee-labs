...
const LABELS = { kp: 'Kp', ki: 'Ki', kd: 'Kd', k: 'K', z: 'zero', p: 'pole' }
...
export function crossingGain(ctrlId, ctrlP, marg) {
  ...
  return { key, label: LABELS[key], now, crossing: now * marg.gainMargin }
}

/** The 0.9× / 1.1× chips read the LIVE margin, so they work from wherever the gain is. */
const marginChips = (s, marg) => {
  ...
  return [0.9, 1.1].map((f) =>
    gain(
      x.key,
      x.crossing * f,
      `${f} × GM → ${x.label} ${fmtNum(x.crossing * f, 4)}`,
...
const crossingChip = (s, marg, title = 'The closed-loop poles land exactly on the imaginary axis') => {
  const x = crossingGain(s.ctrlId, s.ctrlP, marg)
  return x ? [gain(x.key, x.crossing, `${x.label} → ${fmtNum(x.crossing, 4)} (on the axis)`, title)] : []
}
...
export const LESSONS = [
  ...
  },
  {
    ...
    note:
      'The same plant with an integral term added. The error goes to exactly zero: the integral ' +
      'keeps accumulating for as long as any error remains, and stops growing only when there is ' +
      'nothing left to grow on. Not an approximation.',
    try:
      'Ki → 0.2 takes 30 s to settle, Ki → 5 overshoots 23% — and both land on exactly 1. Back ' +
      'to P and the 10% gap returns.',
    featured: ['ki'],
    chips: [
      gain('ki', 0.2),
      gain('ki', 1),
  ...
  },
  {
    ...
    try:
      'switch to PI and the offset is erased exactly — the integral winds up until nothing of ' +
  ...
  },
  {
    ...
    note:
      ...
      'reaches −180° sooner and has less margin before it oscillates. This PI loop has 19° of ' +
      'phase margin; the same Kp under plain proportional control has 52°.',
    try:
  ...
  // --------------------------------------------------- Losing stability
  {
    ...
    note:
      ...
      'they reach −180° while the gain is still above one, and there the feedback that was ' +
      'subtracting starts adding. At Kp = 4 the loop already rings 42%.',
    try:
  ...
  },
  {
    ...
    try:
      '0.9 × GM and the verdict stays stable; 1.1 × GM and it flips. Both chips read the live ' +
      'margin, so they work from wherever Kp is.',
    featured: ['kp'],
  ...
  },
  {
    ...
    note:
      ...
      'where the error is heading rather than where it is, which adds phase exactly where the ' +
      'loop is short of it. Real derivative terms are always filtered, because this also ' +
      'amplifies noise.',
    try:
  ...
  },
  {
    ...
    try:
      ...
      'becomes the trace. Drag the pole down toward the zero and the added phase shrinks.',
