...
${RULES}
READ FIRST: ELECTRONICS_LAB_PLAN.md (§2 engine, §3 models, §4 app and its defaults, §5 the curriculum for Groups ${lane.groups}, in full), apps/electronics-lab/AGENT_BRIEF.md (§3 contracts, §4 the lesson schema and quantity paths, §5 the library netlists with fixed node names, §6 pins), apps/electronics-lab/NEEDS.md §4 and §5 (the transistor symbol, which has landed in packages/ui, and two numbers that moved), and the app as it stands: apps/electronics-lab/src/groups/a.js and c.js and lessons/a.js and c.js (your models), experiments.js, lessons.js, terms.js, math.js, experiments.test.js, App.jsx and components/. Then packages/network/src (bjt, mosfet, companion, smallSignal, transfer, loop, macro, junction, noise and their tests) so every reading path resolves to a solver call. Then packages/ui/src/Schematic.jsx's Transistor component and schematicGeometry.js's transistor exports.
...
VERIFY: npx vitest run apps/electronics-lab --maxWorkers=2 green (existing groups A and C must stay green), npm run build --workspace apps/electronics-lab succeeds. Commit by path on your branch after each group. Return the structured result with ok=true only if all of that holds.`,
