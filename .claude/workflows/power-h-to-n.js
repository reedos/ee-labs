...
VERIFY before you report: npx vitest run apps/power-lab packages/switched --maxWorkers=2 green with Groups A to G untouched and still green; npm run build --workspace apps/power-lab succeeds; node packages/prose/bin/lint.mjs on the brief, NEEDS.md and BACKLOG.md is clean. Return the structured result with ok=true only if all of that holds; list every deviation from the plan's numbers with the engine's value and why.`,
