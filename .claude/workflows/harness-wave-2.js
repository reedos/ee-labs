...
phase('Harness')
const results = await pipeline(LABS, (lab) => agent(`You are a VERIFIER in the EE Labs program, assigned one lab that has no browser harness yet: apps/${lab.slug}. Chromium is installed for Playwright at /opt/pw-browsers (PLAYWRIGHT_BROWSERS_PATH is set; never run "playwright install"; launch with executablePath /opt/pw-browsers/chromium if the installed @playwright/test version does not find it).
