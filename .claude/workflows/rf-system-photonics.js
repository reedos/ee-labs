...
READ: ${plan} in full, ${brief}, CORE_SCOPE.md, STYLE.md, REVIEW_PLAYBOOK.md, PROGRAM.md §3 and §4, and every file the branch added or changed (git diff ${base}...${branch} --stat, then read them). Your scope is ${scope}.
...
VERIFY: npx vitest run apps/rf-lab packages/rf packages/ui --maxWorkers=2 green; npm run build --workspace apps/rf-lab succeeds; the prose lint is clean on every .md you touched. Return the structured result; ok=true only if all of that holds.`, OPTS('rf:1', 'Build', RESULT))
...
VERIFY: npx vitest run apps/rf-lab packages/rf packages/ui --maxWorkers=2 green; npm run build --workspace apps/rf-lab succeeds; the prose lint clean on every .md you touched. Return the structured result.`, OPTS('rf:2', 'Build', RESULT))
      if (!r2) { log('RF second sitting: no result'); return null }
      log(`RF second sitting: ok=${r2.ok}; ${r2.built}`)
      return review('RF Lab, Groups A to D', r2.branch, INTEGRATION, 'RF_LAB_PLAN.md', 'apps/rf-lab/AGENT_BRIEF.md', 'apps/rf-lab, packages/rf and packages/ui/src/SmithCanvas.jsx', 'npx vitest run apps/rf-lab packages/rf packages/ui --maxWorkers=2', 'npm run build --workspace apps/rf-lab', r2)
    },
    async () => {
...
VERIFY: npx vitest run apps/system-lab packages/rf --maxWorkers=2 green; npm run build --workspace apps/system-lab succeeds; the prose lint clean on every .md you touched. Return the structured result.`, OPTS('system:1', 'Build', RESULT))
      if (!s) { log('System sitting: no result'); return null }
      log(`System sitting: ok=${s.ok}; ${s.built}`)
      return review('System Lab, Group A', s.branch, r1.branch, 'SYSTEM_LAB_PLAN.md', 'apps/system-lab/AGENT_BRIEF.md', 'apps/system-lab and the new files in packages/rf the builder named (the RF Lab\'s own files are not yours)', 'npx vitest run apps/system-lab packages/rf --maxWorkers=2', 'npm run build --workspace apps/system-lab', s)
    },
  ])
...
VERIFY: npx vitest run apps/photonics-lab packages/photonics --maxWorkers=2 green; npm run build --workspace apps/photonics-lab succeeds; the prose lint clean on every .md you touched. Return the structured result.`, OPTS('photonics:1', 'Build', RESULT))
...
VERIFY: npx vitest run apps/photonics-lab packages/photonics --maxWorkers=2 green; npm run build --workspace apps/photonics-lab succeeds; the prose lint clean on every .md you touched. Return the structured result.`, OPTS('photonics:2', 'Build', RESULT))
  if (!p2) { log('Photonics second sitting: no result'); return null }
  log(`Photonics second sitting: ok=${p2.ok}; ${p2.built}`)
  return review('Photonics Lab, Groups A, C, D, E and F', p2.branch, INTEGRATION, 'PHOTONICS_LAB_PLAN.md', 'apps/photonics-lab/AGENT_BRIEF.md', 'apps/photonics-lab and packages/photonics', 'npx vitest run apps/photonics-lab packages/photonics --maxWorkers=2', 'npm run build --workspace apps/photonics-lab', p2)
}
