...
${RULES}
${NETWORK}
READ FIRST, in full: ${lab.plan} (its decisions are made, do not reopen them), PROGRAM.md §3 and §4, EE_LABS_MAP.md §2 Track D, then ${lab.read}, then packages/ui and packages/explain as apps/electronics-lab/AGENT_BRIEF.md's tables list them, and that brief as the shape of yours.
...
SETUP: you are in your own git worktree created from master. First run: git checkout ${r2.branch} && (npm ci --no-audit --no-fund || npm install --no-audit --no-fund). You may commit fixes by path on this branch. NEVER push.
${RULES}
The builder reported: ${JSON.stringify(r2)}.
READ: ${lab.plan} in full, apps/${lab.slug}/AGENT_BRIEF.md, CORE_SCOPE.md, STYLE.md, REVIEW_PLAYBOOK.md, PROGRAM.md §3 and §4, and every file the branch added or changed (git diff ${INTEGRATION}...${r2.branch} --stat, then read them).
